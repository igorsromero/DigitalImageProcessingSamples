...
for x in range(cols):
    for y in range(rows):
        if distancia((y, x), center) < 50:
            base[y, x] = 1
plt.subplot(152), plt.imshow(np.abs(base), "gray"), plt.title("Ideal")
...
plt.subplot(153), plt.imshow(np.abs(base), "gray"), plt.title("Butterworth")
...
plt.subplot(154), plt.imshow(np.abs(base), "gray"), plt.title("Gaussiano")
